...
class Game:
    def __init__(self) -> None:
        ...
        self.__where_i_am: Literal['home','settings','game-choose-action','game-choose-atk','game-choose-obj','game-choose-poke','game-action'] = 'game-choose-action'
        ...
        self.__ui: dict[str, GuiLib.Btn | GuiLib.Pokemon | GuiLib.PokemonStatus | GuiLib.Txt | None] = {
            ...
            "p1_poke":GuiLib.Pokemon("p1_poke",self.__p1_x_pos,23,self.__p1.get_id()), # type: ignore # Need this cuz my IDE is mad else
            "p2_poke":GuiLib.Pokemon("p2_poke",self.__p2_x_pos,23,self.__p2.get_id(),True), # type: ignore # Same as line before
            "p1_stat":GuiLib.PokemonStatus("p1_stat",3,2,self.__p1),
            "p2_stat":GuiLib.PokemonStatus("p1_stat",71,2,self.__p2)
        }
    ...
    def update(self) -> None:
        if self.__where_i_am == "game-action":
            ...
            if pyxel.frame_count == self.__action_start_end[0]+45:
                ...
                self.__player_playing.execute_next_turn(Arena().what_is_the_player_that_isnt_playing(self.__round,self.__p1,self.__p2))
            ...
            if pyxel.frame_count == self.__action_start_end[1]:
                ...
                self.__ui["atk_btn"] = GuiLib.Btn("atk_btn",3,58,60,20,'Attaquer',7)
                self.__ui["obj_btn"]=GuiLib.Btn("obj_btn",65,58,60,20,'Objets',7)
                self.__ui["poke_btn"]=GuiLib.Btn("poke_btn",3,80,60,20,'Pokemon',7)
                self.__ui["leave_btn"]=GuiLib.Btn("leave_btn",65,80,60,20,'Fuir',7)
        i = 0
                                ...
                                case 'game-choose-action':
                                    match uie.id:
                                        ...
                                        case 'leave_btn':
                                            ...
                                            for e in ["atk_btn","obj_btn","poke_btn","leave_btn"]:
                                                self.__ui.pop(e)
                                            self.__ui['loose_msg'] = GuiLib.Txt(12,75,f'Vous abandonez, {(self.__round%2)+1} a perdu !',7)
                                        case _:
                                            pass
